...
- **Quirks:** `gates.conf`'s `.venv/bin/python -m pytest -q` fails here with `No such file or directory` (127) — that path is macOS-shaped, and macOS has no bare `python`, so the one literal command cannot serve both machines (logged in `office/inefficiencies/log.md`). `ledger-sync verify` also fails here through its own `.ps1` port parse check (`ledger-state.ps1` breaks on any PowerShell version, `ledger-mem.ps1` on the built-in 5.1) — verify integrity instead with `sha256sum -c MANIFEST.sha256` inside `.context_ledger/core` (70 OK / 0 FAILED, 2026-09-26); pwsh 7.6.6 is installed at `C:\Program Files\PowerShell\7\pwsh`. A stale Kilo worktree exists at `.kilo/worktrees/wistful-piano` (detached HEAD, self-ignored by `.kilo/.gitignore`); it is not part of this repo's tracked tree. No `recordings/` directory in this checkout, so no live Deriv recording is accumulating here.
