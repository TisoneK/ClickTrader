...
| Njeri | S005 | deepseek/deepseek-flash | Registering a Windows-runnable gate command so the mandatory `pre-commit`/`exit` gates pass on Tison's box without breaking bao's Mac | Working | Phase 1 setup: reading `ledger-gates`' command execution and discovery paths before editing `workflows/gates.conf` |
