...
class ForexReplayResult:
    ...
    def verdict(self) -> str:
        ...
        if lo > 0.5:
            return (
                f"POSITIVE directional accuracy, interval entirely above 50% ({lo:.3f}-{hi:.3f}). "
        ...
        if hi < 0.5:
            return f"Worse than a coin flip: interval {lo:.3f}-{hi:.3f}, entirely below 50%."
        return f"No directional edge: hit rate {oos.hit_rate:.3f}, interval {lo:.3f}-{hi:.3f} includes 50%."
    ...
    def report(self) -> str:
        ...
        for seg in (self.in_sample, self.out_of_sample, self.control):
            ...
            lines.append(f"  hit rate   {seg.hit_rate:.3f}  ({ci_label} {lo:.3f}-{hi:.3f}; a coin flip gives 0.500)")
