...
def _no_real_sleep(monkeypatch):
    monkeypatch.setattr(deriv.time, "sleep", lambda _seconds: None)
...
def test_stream_ticks_reconnects_on_a_dropped_connection(monkeypatch):
    ...
    monkeypatch.setattr(deriv, "connect", lambda app_id=deriv.DEFAULT_APP_ID: connections.pop(0))
...
def test_stream_ticks_does_not_retry_api_errors(monkeypatch):
    ...
    monkeypatch.setattr(deriv, "connect", lambda app_id=deriv.DEFAULT_APP_ID: ws)
...
def test_stream_ticks_gives_up_after_retries(monkeypatch):
    ...
    monkeypatch.setattr(deriv, "connect", always_broken)
